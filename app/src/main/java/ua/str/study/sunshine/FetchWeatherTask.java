...
//            int locationIndex = locationCursor.getColumnIndex(WeatherContract.LocationEntry._ID);
    ...
    private String[] getWeatherDataFromJson(String forecastJsonStr, String locationSetting) throws JSONException {
        ...
        try {
            ...
            // add to database
            if ( cVVector.size() > 0 ) {
                int insertCount = mContext.getContentResolver().bulkInsert(WeatherEntry.CONTENT_URI, cVVector.toArray(new ContentValues[cVVector.size()]));
                Log.d(LOG_TAG, "FetchWeatherTask cVVector.size() > 0, insertCount: " + insertCount);
